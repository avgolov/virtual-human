...
#include "../../SystemModels/Systems/WorkLoad/StoreWorkLoad.h"

#include "../../SystemModels/Systems/NervousSystem/WorkLoadNervousSys.h"

namespace Experiments
{
	void IncremetalLoadSysBuilder::CreateNewSystem() {
		...
		system->SetLactateTreshold(config_->get<double>("Model.Blood.Muscle.LactateTreshold")/3.6);
		...
		//Parse experemental data
		io::CSVReader<12> in(config_->get<std::string>("Preprocessing.ExperementalData"));
		std::vector<double> velStorage;
		std::vector<double> brStorage;
		std::vector<double> hrStorage;
		std::vector<double> veStorage;
		double vVal, tVal, vo2Val, vo2kgVal, vco2Val, veVal, 
			   rerVal, feo2Val, eqCo2Val, eqO2Val, brVal, hrVal;

		while (in.read_row(tVal, vVal, vo2Val, vo2kgVal, vco2Val, veVal,
			               rerVal, feo2Val, brVal, eqCo2Val, eqO2Val, hrVal))
		{
			velStorage.push_back(vVal/3.6);
			brStorage.push_back(brVal);
			hrStorage.push_back(hrVal);
			veStorage.push_back(veVal);
		}
				
		//Work model
		auto work = boost::make_shared<StoreWorkLoad>();
		work->SetStorage(velStorage);
		...
		nervous->SetVeStorage(veStorage);
		nervous->SetBrStorage(brStorage);
		nervous->SetQStorage(hrStorage);
		nervous->SetSteadyStateThreshold(config_->get<double>("NumericalMethod.SteadyStateTime"));
		nervous->SetUseExperiment(config_->get<int>("Model.Regulation.UseExperiment") == 1);
