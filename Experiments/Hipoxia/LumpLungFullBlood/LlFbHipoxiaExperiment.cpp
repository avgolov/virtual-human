...
#include "LlFbHipoxiaExperiment.h"
#include "LlFbHipoxiaSysBuilder.h"


namespace Experiments
{
	void LlFbHipoxiaExperiment::Run()
	{
		...
		LlFbHipoxiaSysBuilder builder(&config);
		...
		postprocess.AddHandler(new BasePostprocess<IRespiratorySys, RespiratorySysParam>(new TimeSaveCondition(0.5), human_sys->GetRespiratorySys(), CO2, result_path));
		//postprocess.AddHandler(new BasePostprocess<IRespiratorySys, RespiratorySysParam>(new TimeSaveCondition(0.1), human_sys->GetRespiratorySys(), Vol, result_path));
