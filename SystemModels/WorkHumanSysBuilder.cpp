...
#include "../Systems/RaspiratorySystem/MiptNet1DRaspSys.h"
#include "../Systems/BloodSystem/AvgTanksBloodSys.h"
...
namespace SystemModels
{
	...
	{
		auto raspSys = std::make_shared<MiptNet1DRaspSys>();
		raspSys->SetParam(RateSS, 10);
		raspSys->SetParam(TidalVolumeSS, 500.);
		raspSys->SetParam(Rate, 10);
		raspSys->SetParam(TidalVolume, 500.);
		ResultHandlerUtility resultsUtility;
		auto alveoles = raspSys->GetAlveoles();
		auto tree = raspSys->GetTree();
		...
		raspSys->SetResultutility(resultsUtility);

		auto bloodSys = std::make_shared<AvgTanksBloodSys>();
		...
		auto tissueSys = std::make_shared<BaseTissueSys>();
		...
		auto nervousSys = std::make_shared<BloodDivNervousSys>();

		auto system = std::make_shared<HumanSys>();
		system->SetRaspiratorySys(raspSys);
